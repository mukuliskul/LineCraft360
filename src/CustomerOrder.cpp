...
#include <iomanip>
#include <iostream>
#include "../include/CustomerOrder.h"
...
namespace myproject
{
	...
	CustomerOrder::CustomerOrder(const std::string &str) : CustomerOrder()
	{
		...
		while (more) // to count the total number of items to init double pointer array
		{
			if (m_cntItem == 0)
			{
				p = next_pos; // to store current pos of start of items
			}
	...
	void CustomerOrder::fillItem(Station &station, std::ostream &os)
	{
		...
		for (size_t i = 0; i < m_cntItem; i++)
		{
			...
			{
				breakLoop = true; // break loop to only change single item
